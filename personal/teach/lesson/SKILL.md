...
name: lesson
description: >
  Menu launcher for this repo's Laravel 12→13 lessons: lists what is in lessons/, proposes the
  first not-yet-done one, and hands the teaching session to /teach. Read-only on progress — it
  never marks completion. Use whenever the learner wants to start, resume, or pick a lesson, even
  without naming one. Not for creating or refreshing lessons: that is lesson-update.
...
Triggers in either language — "do lesson 3", "facciamo la lezione 3", "what's the next lesson",
"qual è la prossima lezione", "resume", "riprendiamo", "show me the lessons", "teach-lesson"
(its former name).
...
2. **One final action: hand off to `/teach`.** Build the menu, resolve the lesson, then
   ...
   course-page warm-up is a best-effort side step; the hand-off stays the final act.
...
- **Version**, for every row: read it from the `> Version:` line in the lesson's frontmatter/prose
  (e.g. `12.x, through 12.19` → show as `12.x–12.19`; a single version stays as-is).
...
Once the lesson is resolved, warm up the course page so the learner can follow along in the
browser while `/teach` works — best-effort, never blocking. Procedure:
[references/course-page.md](references/course-page.md).
...
If step 5 deferred the browser open because the lesson `.html` did not exist yet, open it the
first time `/teach` writes `lessons/<slug>.html` — once, same opener logic. Still missing when
the session ends: leave it, the learner already has the URL.
...
- **Number > 12** (`13`, `14`…) → this is the **display id**, not a filename: recompute the
  ...
  `(number - 12)`-th entry. `13` is always the earliest not-yet-superseded version-pure lesson,
  `14` the next — never assume it maps to a file literally named `13-*.md` (it doesn't).
- **Version string** (`13.17`, `13.17.0`) → resolve to the matching active
  ...
  path. No match → fall back to the menu.

An already-done lesson, however it was reached: warn once (`⚠️ Lesson 03 is already done —
re-running.`) then hand off anyway. No extra confirmation.
