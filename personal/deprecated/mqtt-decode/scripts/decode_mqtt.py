...
def decode_body(cmd: int, body: bytes) -> list[tuple[str, bytes, str]]:
    ...
    # ── Fallback: raw hex ───────────────────────────────────────────────────────
        ...
        if body:
            rows.append(("Body (raw)", body, f"{len(body)} byte — {fmt_hex(body)}"))
...
def decode(hex_string: str) -> str:
    # Normalize: if no spaces (raw hex like "303030..."), insert space every 2 chars
    stripped = hex_string.strip()
    if ' ' not in stripped and len(stripped) > 2:
        stripped = ' '.join(stripped[i:i+2] for i in range(0, len(stripped), 2))
